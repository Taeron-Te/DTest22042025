            ...
            case["Match"]:
                ...
                while True:
                    ...
                    if tmp == "":
                        break
                    lincs.append(tmp)
                c.match_generate(input("text"), a, lincs, input("filename"))
            case["FL"]:
                antecedents = {
                    'oil_pressure': {
                        'range': [0, 100, 1],
                ...
                while True:
                    ...
                    if tmp == "":
                        break
                    lincs.append(tmp)
                c.fuzzy_generate(input("filename"), antecedents, consequents, rule_definitions, lincs)

            case["get_all"]:
                c.get_generated_files()
            case["starter"]:
                c.starter_generate(input("filename"), input("linkA"))
            case["ender"]:
                c.ender_generate(input("filename"), input("verdict"))
            case ["end"]:
                break
